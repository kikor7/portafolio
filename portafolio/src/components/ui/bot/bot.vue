...
// Controlar si el chat está abierto o cerrado
...
// Las opciones clicables que se le muestran al usuario
const opciones = ref([
...
// Función que maneja el clic en las opciones
const seleccionarOpcion = async (idOpcion: string, textoOpcion: string) => {
  ...
  // 2. Respuesta instantánea del bot según lo que haya clicado
  setTimeout(async () => {
    let respuestaBot = ''
    ...
    // Scroll automático tras responder el bot
    await nextTick()
    hacerScroll()
  }, 400) // Un mini retraso de 400ms para que parezca que "piensa" y quede más natural
}

// Función auxiliar para mantener la vista abajo si hay muchos mensajes
const hacerScroll = () => {
...
<template>
  <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
    ...
    <Card 
      v-if="isOpen" 
      class="w-80 sm:w-96 h-[500px] flex flex-col mb-4 shadow-2xl border-slate-200 animate-in slide-in-from-bottom-5 duration-200 rounded-2xl bg-white"
    >
      <CardHeader class="p-4 border-b flex flex-row items-center gap-2 space-y-0 bg-slate-50 rounded-t-2xl">
        <div class="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
        <CardTitle class="text-xs font-bold text-slate-700 uppercase tracking-wider">Kiko Assistant</CardTitle>
      </CardHeader>
      ...
      <CardContent class="flex-1 overflow-hidden p-0 flex flex-col">
        ...
        >
          <div 
            ...
            :key="msg.id"
            :class="['flex text-xs p-3 rounded-2xl max-w-[85%] leading-relaxed shadow-sm', 
              msg.emisor === 'bot' 
                ? 'bg-slate-100 text-slate-800 rounded-bl-none mr-auto border border-slate-200/60' 
      ...
      <div class="p-3 border-t bg-slate-50/50 flex flex-col gap-2 rounded-b-2xl">
        <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider px-1">Selecciona una opción:</span>
        <div class="flex flex-col gap-1.5">
          <Button 
            v-for="opcion in opciones" 
            :key="opcion.id"
            variant="outline"
            class="justify-start text-xs h-8 font-medium border-slate-200 bg-white hover:bg-slate-100 hover:text-indigo-600 shadow-sm transition-colors text-left truncate rounded-xl"
